...
# ╔═╡ d2cd16c6-361f-11ef-02bd-111c455191ec
using PlutoUI

# ╔═╡ 8896370f-3226-460b-9101-8a711588cc92
using VLBIData

# ╔═╡ 757d3476-dbb0-4b51-b1a4-4f8929ce7c88
using Combinatorics

# ╔═╡ 9a1e41ef-16b5-400f-b4cf-3f1ec564d8a1
using CairoMakie

# ╔═╡ bcc19ff6-aa02-4b8e-9629-bf5d7ff32bea
using Dates

# ╔═╡ d8f4f62a-6093-4e0b-b243-77f304f8215c
using HypertextLiteral

# ╔═╡ bd260114-336c-4dc3-9225-366b07243522
using DSP

# ╔═╡ 69a31849-5815-4fbc-8d52-b69bee4cd252
using Statistics
...
md"""
This notebook aims to interactively plot multiple sources by baseline from the data read from per-source uvfits files.

The data (one uvfits file per source) are assumed to be located at **"<basepath>/hops-<band>/<stage>/<epoch>"** following the EHT-HOPS pipeline organization.
"""
...
md"""
Absolute path (or relative path to **$(basepath)**) in which to save output: $(@bind outdir confirm(TextField(default=	"plots"), label="Submit"))
"""

# ╔═╡ c5fa4a85-f3a0-4e05-9c10-b2996c67acdb
md"""
The output will be saved to **$(savepath = joinpath(basepath, outdir))**
"""

# ╔═╡ c9fc5e19-d879-4ab8-9f7d-b78df00fe6c9
@bind sel confirm(PlutoUI.combine() do Child
	md"""
	Select data to plot:
...
# ╔═╡ d1f4832f-a9b6-498b-9ac0-9e11bfa180c8
# code block to extract all source names
function getsources(basepath, band, stage, epoch)
	currpath = joinpath(basepath, "hops-"*band, stage, epoch)
	uvfiles = [joinpath(currpath, f) for f in readdir(currpath)]

	regex_pattern = "hops_$(epoch)_(.+?)(?:\\.apriori)?\\+avg\\.uvfits"
	regex = Regex(regex_pattern)

	# Iterate over the filenames and extract the target part
	srclist = []
	for uvf in uvfiles
    	m = match(regex, uvf)
    	if m !== nothing
        	push!(srclist, m.captures[1])
    	end
	end
	return srclist
end

# ╔═╡ 81dfc74f-b04a-4300-82d0-ff06fdc59fca
function getbaselines(basepath, band, stage, epoch, sources)
	currpath = joinpath(basepath, "hops-"*band, stage, epoch)
	uvfiles = [joinpath(currpath, f) for f in readdir(currpath)]
	# load the necessary data structures
	srcdict = Dict()
	for source in sources
		fname = first(filter(f -> occursin(source, f), uvfiles))
		tempdict = Dict()
		tempdict["uvfits"] = VLBI.load(fname)
		tempdict["vistables"] = VLBI.table(tempdict["uvfits"])
		srcdict[source] = tempdict
	end

	# construct a baseline mapping between antenna ids and names
	allbaselines = nothing
	iter = 0
	for source in collect(keys(srcdict))
		ant_id = map(x -> x.id, srcdict[source]["uvfits"].ant_arrays[1].antennas)
		ant_name = map(x -> x.name, srcdict[source]["uvfits"].ant_arrays[1].antennas)
		ant_dict = Dict(zip(ant_name, ant_id))
		#println(ant_dict)
		baseline_name_tuples = [Tuple(x) for x in collect(combinations(sort(collect(keys(ant_dict))), 2))]
		baselinedict = Dict()
		for baseline in baseline_name_tuples
			baselinedict[baseline] = (ant_dict[baseline[1]], ant_dict[baseline[2]])
		end
		#println(keys(baselinedict))
		if iter == 0
			allbaselines = keys(baselinedict)
			iter = 1
		else
			allbaselines = union(allbaselines, keys(baselinedict))
		end
		# save to source Dict
		srcdict[source]["baselines"] = baselinedict
	end
	return srcdict, sort(collect(allbaselines))
end

# ╔═╡ 47b4dbd4-18e6-47da-9a26-0931d2325728
begin
sources = getsources(basepath, sel.band, sel.stage, sel.epoch)
srcdict, allbaselines = getbaselines(basepath, sel.band, sel.stage, sel.epoch, sources)
end;

# ╔═╡ 381f86b8-be58-4f9a-871c-fe99d915e13c
@bind p confirm(PlutoUI.combine() do Child
	md"""
	Make further selection:

	**Sources** $(Child("sources", MultiCheckBox(sources, select_all=true)))

	**Baselines** $(Child("baselines", MultiCheckBox(allbaselines, select_all=true)))
	
	**Polarization products** $(Child("stokes", MultiCheckBox([:LL, :LR, :RL, :RR], select_all=true)))

	**X-axis** $(Child("xaxis", Select([:time])))
	**Y-axis** $(Child("yaxis", Select([:amplitude, :phase])))
	"""
...
function plot_by_baseline(srcdict, band, stage, epoch, sources, baselines, stokes, xq, yq, savepath; unwrap=true, savefig=false)
	...
	if yq ∉ (:amplitude, :phase)
		throw(ArgumentError("Invalid value for yq: $yq. Exiting."))
	end
	...
	for baseline in baselines
		...
		# one stokes quantity per subplot
		for (plotcol, pp) in enumerate(stokes)
			ax = CairoMakie.Axis(f[plotcol,1], title="Stokes $pp", width=800, height=400)
			lgd_els = []
			valid_sources = []
			for (n, source) in enumerate(sources)
				try
					# compute the requested yquantity
					if yq == :amplitude
						data = map(v -> abs(v.visibility), filter(v -> v.stokes ∈ (pp,) && v.baseline.ants_ix == srcdict[source]["baselines"][baseline], srcdict[source]["vistables"]))
...
manifest_format = "2.0"
project_hash = "297222598c6b6b93bc2a0ad00491b3f9ccfce669"
...
git-tree-sha1 = "27f1bf327680657ddd0c0b863b1c0488083ab50b"
...
[[deps.DataPipes]]
git-tree-sha1 = "79843dda2bbd58d62123027d38051a1bf0b646eb"
...
git-tree-sha1 = "585992f1464c98a1cac68155290f8504d50ff9e2"
